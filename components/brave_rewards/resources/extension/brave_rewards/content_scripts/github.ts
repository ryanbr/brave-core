...
//const getCommentMetaData = (comment: Element): RewardsTip.GitHubCommentMetaData | null => {
//  if (!comment) {
//    return null
//  }
//
//  const commentTextElements = comment.getElementsByClassName('comment-text')
//  if (!commentTextElements || commentTextElements.length === 0) {
//    return null
//  }
//
//  const commentText = commentTextElements[0] as HTMLElement
//
//  const commentTimestampElements = comment.getElementsByClassName('js-short-timestamp')
//  if (!commentTimestampElements || commentTimestampElements.length === 0) {
//    return null
//  }
//
//  const commentTimestamp = commentTimestampElements[0].getAttribute('data-time') || ''
//
//  return {
//    name: comment.getAttribute('data-name') || '',
//    screenName: comment.getAttribute('data-screen-name') || '',
//    userId: comment.getAttribute('data-user-id') || '',
//    commentId: comment.getAttribute('data-comment-id') || '',
//    commentTimestamp: parseInt(commentTimestamp, 10) || 0,
//    commentText: commentText.innerText || ''
//  }
//}
...
const configureGitHubCommentTipAction = (commentClass: string) => {
  const comments = document.getElementsByClassName(commentClass)
  console.log(comments.length)
  for (let i = 0; i < comments.length; ++i) {
    const actions = comments[i].getElementsByClassName('timeline-comment-actions')[0]
    if (actions) {
      ...
      if (tippingEnabled && braveTipActions.length === 0) {
        actions.appendChild(createBraveTipAction(comments[i]))
...
const configureGitHubTipAction = (rewardsEnable: boolean, inlineTipsEnalbed: boolean) => {
    if (!(rewardsEnable && inlineTipsEnalbed)) {
        return
    }
    const commentClasses = ['timeline-comment-group']

    for (let commentClass of commentClasses){
      configureGitHubCommentTipAction(commentClass)
    }
}
...
const configureBraveTipAction = () => {
    ...
    chrome.runtime.sendMessage('rewardsEnabled', function (rewards) {
      ...
      chrome.runtime.sendMessage(msg, function (inlineTip) {
        configureGitHubTipAction(rewards.enabled, inlineTip.enabled)
      })
    })
